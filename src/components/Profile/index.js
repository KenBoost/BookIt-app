      ...
      <div className="reserva-cards">
        {reservas.map((reserva) => (
          <div key={reserva._id} className="reserva-card">    
            ...
            <p>Devolver antes del {reserva.fecha_devolucion}</p>
            {reserva.libro_info && (
              <div className="libro-info">
                <p>Libro seleccionado</p>
                <p style={{ fontSize: '1.6rem', fontWeight: 'bold' }}>{reserva.libro_info.titulo}</p>
                <p>Autor: {reserva.libro_info.autor}</p>
                <p>Género: {reserva.libro_info.genero}</p>
                {/* Otros campos de información del libro */}
              </div>
            )}
            <p style={{ color: '#00b398',fontSize: '1.6rem', fontWeight: 'bold' }}>Esta reserva está {reserva.estado}</p>
